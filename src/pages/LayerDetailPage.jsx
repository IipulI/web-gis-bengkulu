...
// Leaflet imports (Biarkan jika nanti ingin menambah peta)
...
const getExportFilename = (layerName, format) => {
  ...
  switch (format) {
    case "shp": return `${safeName}.zip`;
    case "geojson": return `${safeName}.geojson`;
    case "kml": return `${safeName}.kml`;
...
const LayerDetailPage = () => {
  ...
  const [size, setSize] = useState(10); // Default rows per page
  ...
  // 2. Query Attachments (FIXED: Added arrow function)
  const { data: attachmentData, isLoading: attachmentLoading } = useQuery({
    queryKey: ["attachments", id, viewFeatureId],
    queryFn: () => attachmentService.getAttachment(id, viewFeatureId), // Arrow function added
  ...
  const updateMutation = useMutation({
    mutationFn: ({ featureId, data }) => {
      ...
      return layerService.updateSpatialItem(id, featureId, payload);
    },
  ...
  const getFeatureValue = (feature, fieldDef) => {
    if (!feature || !fieldDef) return "";

    if (fieldDef.path === "root") {
      return feature[fieldDef.key];
    }

    return feature.properties?.[fieldDef.key];
  };
  ...
  const columns = useMemo(() => {
    ...
    return schema.definition.map((field) => ({
      ...
      editable: field.editable !== false, // Default true
      originalDefinition: field,
      render: (feature) => getFeatureValue(feature, field) ?? "-",
    }));
  }, [schema]);
      ...
      <DashboardLayout>
        ...
        <div className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 tracking-tight">
              Layer: <span className="text-green-600">{layer?.name || "Loading..."}</span>
            </h1>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="p-6 border-b border-gray-50 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <h2 className="font-bold text-gray-800 text-lg">Daftar Atribut Data</h2>
          ...
          <div className="overflow-x-auto">
            {features.length === 0 ? (
                <div className="py-20 flex flex-col items-center justify-center text-gray-400">
                  ...
                  <p className="text-sm font-medium">Belum ada data yang tersedia</p>
                </div>
            ) : (
                <table className="w-full text-sm">
                  ...
                  <tr className="bg-gray-50/50 text-gray-500 font-semibold uppercase tracking-wider text-[11px]">
                    {columns.map((col) => (
                        <th key={col.key} className={`p-4 text-${col.align || "left"} border-b border-gray-100`}>
                          {col.label}
                        </th>
                    ))}
                    <th className="p-4 text-center border-b border-gray-100">Aksi</th>
                  </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-50">
                  {filteredFeatures.map((f) => (
                      <tr key={f.id} className="hover:bg-green-50/30 transition-colors group">
                        {columns.map((col) => (
                            <td key={col.key} className={`p-4 text-gray-600 text-${col.align || "left"}`}>
          ...
          {pagination && (
              <div className="p-6 bg-slate-50/50 border-t border-slate-100 flex flex-col md:flex-row justify-between items-center gap-4 rounded-b-2xl">
              ...
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                Rows per page:
                  ...
                  <select
                      ...
                      className="bg-white border border-slate-200 rounded-lg px-2 py-1 text-xs font-bold text-slate-600 outline-none focus:ring-2 focus:ring-emerald-500/20 transition-all cursor-pointer"
                  >
                ...
                <div className="flex items-center gap-6">
                  <p className="text-xs font-bold text-slate-500 uppercase tracking-tighter">
                    Page <span className="text-slate-900">{pagination.currentPage}</span> / {pagination.totalPage}
                  </p>
                  <div className="flex gap-2">
                    <button
                        disabled={page === 1}
                        onClick={() => setPage((p) => Math.max(1, p - 1))}
                        className="p-2 bg-white border border-slate-200 rounded-xl disabled:opacity-30 hover:bg-emerald-50 hover:text-emerald-600 hover:border-emerald-200 transition-all shadow-sm active:scale-95"
                    >
              ...
              <div className="bg-white w-full max-w-2xl rounded-3xl shadow-2xl relative overflow-hidden animate-in fade-in zoom-in duration-200">
                ...
                >
                  {columns
                      .filter((c) => c.editable)
                      .map((col) => {
                        const fieldDef = col.originalDefinition;
                        ...
                        return (
                            <div key={col.key} className="space-y-1.5">
                              ...
                              <input
                                  ...
                                  value={getFeatureValue(editForm, fieldDef) ?? ""}

                                  onChange={(e) => {
                                    const newVal = e.target.value;
                                    const updated = structuredClone(editForm); // Deep copy

                                    if (fieldDef.path === "root") {
                                      updated[fieldDef.key] = newVal;
                                    } else {
                                      if (!updated.properties) updated.properties = {};
                                      updated.properties[fieldDef.key] = newVal;
                                    }
                  ...
                  <div className="col-span-full flex justify-end gap-3 pt-4">
                    ...
                    <button
                        ...
                        className="px-6 py-2.5 text-sm font-semibold bg-green-600 text-white rounded-xl shadow-lg shadow-green-200 hover:bg-green-700 active:scale-95 transition-all flex items-center gap-2"
                    >
        ...
        {/* MODAL UPLOAD GAMBAR (Ditambahkan agar fitur Upload berfungsi) */}
            ...
            <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
              ...
              <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl relative overflow-hidden animate-in fade-in zoom-in duration-200">
                <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center">
                  <h3 className="font-bold text-gray-800">Upload Lampiran</h3>
                  <button onClick={() => setUploadModalOpen(false)}><X className="w-5 h-5 text-gray-400"/></button>
                </div>
                <div className="p-6">
                  <div className="border-2 border-dashed border-gray-200 rounded-2xl p-8 flex flex-col items-center justify-center text-center hover:border-green-400 hover:bg-green-50/30 transition-all cursor-pointer relative">
                    <input
                        type="file"
                        accept="image/*"
                  ...
                  <button
                      disabled={!imageFile || uploadAttachmentMutation.isPending}
                      onClick={() => uploadAttachmentMutation.mutate({ layerId: id, featureId: uploadFeatureId, file: imageFile })}
                      className="mt-6 w-full py-3 bg-green-600 text-white font-bold rounded-xl shadow-lg shadow-green-200 hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed flex justify-center items-center gap-2"
                  >
                    ...
                    <div className="space-y-4">
                      {selectedFeature ? (
                          ...
                          <div className="animate-pulse space-y-4">
                            {[1, 2, 3].map((i) => <div key={i} className="h-20 bg-gray-100 rounded-2xl"></div>)}
                          </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="w-full md:w-7/12 overflow-y-auto p-8 bg-white custom-scrollbar">
                    ...
                    {attachmentLoading ? (
                        <div className="h-64 flex flex-col items-center justify-center space-y-4">
                          ...
                          <p className="text-gray-400 text-sm font-medium">Memuat aset...</p>
                        </div>
                        ...
                        <div className="h-64 border-2 border-dashed border-gray-100 rounded-[2rem] flex flex-col items-center justify-center text-gray-300">
                          ...
                          <p className="text-sm font-medium">Tidak ada lampiran gambar</p>
                        </div>
                    ) : (
